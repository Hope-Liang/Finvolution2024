...
class ProjectionHead(nn.Module):
    ...
    def __init__(
            self,
            in_shape: int,
            conv_channels: Sequence[int] = (64, 64, 32, 32),
            dense_neurons: Sequence[int] = (64, 32, 2),
            use_poolings: Sequence[bool] = (True, False, True, False),
            kernel_size: int = 5,
            pool_size: int = 5,
        ...
        ln_shape = (projection_channels, self._get_time_shape(in_shape[1], ksize=5))
        layers = [_get_conv1d_layer(
            in_shape[0],
            projection_channels,
            ln_shape=ln_shape,
            kernel_size=5,
        ...
        for in_channels, out_channels, use_pooling, use_pooling_shape in zip(
            conv_channels[:-1],
            conv_channels[1:],
            use_poolings,
            use_poolings_shape,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self._encoder(x)
        x = self._flatten(x)
        #x = F.avg_pool3d(x, kernel_size=x.size()[2:]).view(x.size()[0], -1)
        return self._head(x)

    def encoder(self, x: torch.Tensor) -> torch.Tensor:
        return torch.randn(250)

    def inference(self, features: torch.Tensor) -> float:
        return np.argmax(self.forward(features).cpu().tolist(), axis=1)
