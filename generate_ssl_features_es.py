...
_DEFAULT_PATH = f'../../datasets/FinvCup2024/Spanish'
_DEFAULT_FEATURE_PATH = f'../../datasets/FinvCup2024/Spanish/{split}_feature_{model_name}'
...
wav_paths = pd.read_csv(os.path.join(_DEFAULT_PATH, 'es_1k.csv'))['path'].to_list()
wav_paths.sort()
...
for wav_path in tqdm.tqdm(wav_paths):
    ...
    wav_name = wav_path[:-4]
    full_feature_path = os.path.join(_DEFAULT_FEATURE_PATH, wav_name.replace('/', '_')+'.npy')
    ...
    if model_name in ['w2vb2', 'w2v2']:
        ...
        for i in range(len(features)):
            ...
            full_feature_path_i = os.path.join(folder_path_i, wav_name.replace('/','_')+'.npy')
